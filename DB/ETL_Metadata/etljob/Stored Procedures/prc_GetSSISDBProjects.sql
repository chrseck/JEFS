...
-- Description: Gets Updates and New enties for Projects from SSISDB
-- =============================================
CREATE PROCEDURE [etljob].[prc_GetSSISDBProjects]
AS
SET NOCOUNT ON
...
WHEN NOT MATCHED
	THEN
		...
		VALUES (
			source.Projectname
			,source.[SSISDBproject_ID]
			,source.FolderID
			)
WHEN MATCHED
	THEN
		UPDATE
		SET [SSISDBproject_ID] = source.SSISDBproject_ID,
		DateDeleted = NULL
